...
aws ec2 authorize-security-group-ingress --group-name wordpress-sec-group --protocol tcp --ipRanges  --cidr 0.0.0.0/32

aws ec2 describe-instances --query "Reservations[*].Instances[*].{InstanceId:InstanceId, PublicIP: PublicIpAddress, State: State.Name}"

#Filter nach Name
aws ec2 describe-instances --filters "Name=tag:Name,Values=mysql" --query "Reservations[*].Instances[*].{PublicIP: PublicIpAddress}"
